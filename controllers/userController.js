...
const userController = {
    ...
    updateUser: async (req, res) => {
        try {
            const { nombre, correo, esEmpresa, id_rol } = req.body;

            // Validar que solo admins puedan asignar rol admin
            if (id_rol === 3) {
                if (!req.session.usuario || req.session.usuario.id_rol !== 3) {
                    return res.status(403).json({ message: 'No autorizado para asignar rol admin' });
                }
            }
